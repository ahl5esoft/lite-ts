...
import { GeoAddMessage, IORedis as Self } from '../../../plugin/redis';
...
let client: IORedis.Redis, subClient: IORedis.Redis, self: Self;

describe('plugin/redis/ioredis.ts', (): void => {
    after((): void => {
        client.disconnect();
    ...
    describe('.publish(channel: string, message: any): Promise<number>', (): void => {
        ...
        it('string', async (): Promise<void> => {
            await subClient.subscribe(channel);

            const task = new Promise((s): void => {
                subClient.on('message', (_, message: string): void => {
                    s(message);
                });
            });
            ...
            strictEqual(res, message);
        });

        it('other', async (): Promise<void> => {
            await subClient.subscribe(channel);

            const task = new Promise(s => {
                subClient.on('message', (_, message: string): void => {
                    s(message);
                });
            });
    ...
    describe('.subscribe(channels: string[], callback: (message: string) => void): Promise<number>', (): void => {
        const channel = 'test-subscribe';
        it('ok', async (): Promise<void> => {
            const task = new Promise((s): void => {
                self.subscribe([channel], s);
            });
            await client.publish(channel, 'hello');
            await client.publish(`${channel}-invalid`, 'hello2');
