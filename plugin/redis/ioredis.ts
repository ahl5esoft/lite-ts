...
export class IORedis extends RedisBase {
    private m_Client: ioRedis.Cluster | ioRedis.Redis;
    private m_Observers: { [key: string]: (message: string) => void; } = {};
    private m_SubClient: ioRedis.Cluster | ioRedis.Redis;
    ...
    public constructor(arg: ioRedis.RedisOptions | ioRedis.ClusterNode[]) {
        ...
        this.m_SubClient = arg instanceof Array ? new ioRedis.Cluster(arg) : new ioRedis(arg);
        this.m_SubClient.on('message', (channel: string, message: string): void => {
            if (this.m_Observers[channel])
                this.m_Observers[channel](message);
        });
    }

    public close(): void {
        this.m_Client.disconnect();
        this.m_SubClient.disconnect();
    }
    ...
    public publish(channel: string, message: any): Promise<number> {
        ...
        return this.m_Client.publish(channel, message);
    }
    ...
    public async subscribe(channels: string[], callback: (message: string) => void): Promise<number> {
        channels.forEach((r): void => {
            this.m_Observers[r] = callback;
        });
        return await this.m_SubClient.subscribe(...channels);
    }
